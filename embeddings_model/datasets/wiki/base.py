...
class WikiDataModule(BaseDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        self.data = pd.read_csv(self.data_dir / (self.annotation_column + '_annotated_comments.tsv'), sep='\t')
        self.data = self._remap_column_names(self.data)

        self.annotations = pd.read_csv(self.data_dir / (self.annotation_column + '_annotations.tsv'), sep='\t')
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        self.text_id_idx_dict = data.loc[:, ['text_id']].reset_index().set_index('text_id').to_dict()['index']
